...
export default class Cards extends React.Component {
    ...
    render() {
        ...
        const loop = data => data.map(item => {
            if (item.children) {
                return (
                    <TreeNode title={item.name} categoryName={item.categoryName} category={item.category} path={item.path} type={item.type} key={item.key}>{loop(item.children)}</TreeNode>
                );
            }
            return (
                <TreeNode title={item.name} categoryName={item.categoryName} category={item.category} path={item.path} type={item.type} key={item.key} isLeaf={true}/>
            );
        });

        const queryTreeNodes = loop(queryTreeData);
        const mutationTreeNodes = loop(mutationTreeData);
                    ...
                    startupLoading
                        ...
                        : (
                            <div>
                                ...
                                <Card key="mutation-card" title={mutationType.name} style={{width: 400}}>
