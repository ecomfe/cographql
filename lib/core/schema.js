    ...
    constructor(schemaFilePath, resolvers = {}, mocks = {}) {
        ...
        const mockType = (type, typeName, fieldName) => {
            return (root, args, context, info) => {
                ...
                if (proxy.defaultScalarMap.has(fieldType.name)) {
                    return proxy.defaultScalarMap.get(fieldType.name)(root, args, context, info);
                }
        ...
        forEachField(this.executableSchema, (field, typeName, fieldName) => {
            const isOnQueryType = this.executableSchema.getQueryType() ? (this.executableSchema.getQueryType().name === typeName) : false;
            const isOnMutationType = this.executableSchema.getMutationType() ? (this.executableSchema.getMutationType().name === typeName) : false;
